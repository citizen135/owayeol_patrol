...
void loop() {
  ...
  uint32_t t = millis();
  updateTime();
  if((t-tTime[0]) >= (30)) {  //frequency_rate
  ...
  // rate controler
  if((t-tTime[1]) >= (150)) {   //frequency_controller
  ...
  // update odometry
  if((t-tTime[2]) >= (150)) {    //frequency_odometry
    float dt, dx, dy;
    float qw, qx, qy, qz;
  ...
  if((t-tTime[3]) >= (150)) {   //frequency_rospinonce
    nh.spinOnce();
    tTime[3] = t;
  }
}
